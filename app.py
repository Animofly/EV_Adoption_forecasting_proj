...
forecast_type = st.radio(
    ...
    ["Cumulative Forecast", "Monthly Forecast"],
    horizontal=True
)

# === Plot based on forecast type ===
if forecast_type == "Cumulative Forecast":
    ...
    st.subheader(f"📊 Monthly EV Forecast for {county} County")

    # Prepare monthly (not cumulative) data
    monthly_actual = county_df[['Date', 'Electric Vehicle (EV) Total']].copy()
    monthly_actual['Source'] = 'Historical'

    monthly_forecast = pd.DataFrame(future_rows)
    monthly_forecast.rename(columns={"Predicted EV Total": "Electric Vehicle (EV) Total"}, inplace=True)
    monthly_forecast['Source'] = 'Forecast'

    monthly_combined = pd.concat([monthly_actual, monthly_forecast], ignore_index=True)

    fig, ax = plt.subplots(figsize=(12, 6))
    for label, data in monthly_combined.groupby('Source'):
        ax.plot(data['Date'], data['Electric Vehicle (EV) Total'], label=label, marker='o')
    ax.set_title(f"Monthly EV Trend - {county} (3 Years Forecast)", fontsize=14, color='white')
    ax.set_ylabel("Monthly EV Count", color='white')
